...
static int test_rtmp_amf_encode(void)
{
...
static int test_rtmp_amf_decode(void)
{
	...
	mb = mbuf_packet(amf_connect, sizeof(amf_connect));

	err = odict_alloc(&dict, 32);
	if (err)
		goto out;

	err = amf_decode(dict, mb);
	TEST_ERR(err);
